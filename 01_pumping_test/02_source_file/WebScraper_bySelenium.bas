...
'2020/12/18 parsing table by selenium
...
Function get_weather_data_bySelenium(ByVal nYear As Integer, ByVal nArea As Integer) As String()
    ...
    For j = 0 To 29
        ...
        x = 1
        For Each tr In bot.FindElementByClass("table_develop").FindElementByTag("tbody").FindElementsByTag("tr")
            i = 1
            If x = 32 Then
...
Sub get_30year_data(nMethod As Integer)
    ...
    Else
        resOut = get_weather_data_bySelenium(nYear, nArea)
    End If
