...
Function getFreezingRow() As Integer
...
'initial setting
Private Function preProcess(nsheet As Integer) As Integer()

    Dim arrCellCopyMode() As Integer, i As Integer
    
    ReDim arrCellCopyMode(1 To 26)
    
    '원가계산서
    If nsheet = 1 Then
        For i = 1 To 26
            arrCellCopyMode(i) = ccmFormula
        Next i
        
        arrCellCopyMode(getIndex("g")) = ccmValue
        arrCellCopyMode(getIndex("h")) = ccmValue
        Exit Function
    End If
    ...
    If nsheet = 2 Then
        ...
        arrCellCopyMode(getIndex("e")) = ccmValue
        arrCellCopyMode(getIndex("f")) = ccmValue
        Exit Function
    End If
    ...
    If nsheet = 3 Then
        ...
        arrCellCopyMode(getIndex("e")) = ccmValue
        arrCellCopyMode(getIndex("f")) = ccmValue
        arrCellCopyMode(getIndex("i")) = ccmValue
        arrCellCopyMode(getIndex("k")) = ccmValue
        arrCellCopyMode(getIndex("m")) = ccmValue
        Exit Function
    End If
    
    preProcess = arrCellCopyMode
    
    'Debug.Print arrCellCopyMode(getIndex("I"))
...
Private Sub copy_one_cell(nsheet As Integer, row As Integer, col As Integer)

    Dim arrCellCopyMode() As Integer
    
    arrCellCopyMode = preProcess(nsheet)
    
    If Cells(row, col).Value = "" Then
        Exit Sub
    End If
    
    If arrCellCopyMode(col) = ccmValue Then
...
Private Sub insert_row2(nrow As Long)
    nrow = nrow + 1
    Range(nrow & ":" & nrow).Select
    Selection.Insert Shift:=xlDown, CopyOrigin:=xlFormatFromLeftOrAbove
End Sub
